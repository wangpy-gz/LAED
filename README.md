...
|---|---|---|
| `Run_Results/` | Default output root for the generic main launcher. It contains per-dataset, per-round LAED outputs; child-process stdout, stderr, and merged logs; batch summaries; and three-round aggregate metrics. The existing `latest_batch_run_summary.json` records GPT-4o for its archived runs. 
|`latest_batch_run_summary.json`, `latest_three_run_average_metrics.{json,csv,md}`, and `command_logs/` |
